...
console.log("buyer balance:", formatEther(await pub.getBalance({address:buyer.address})), "MON");
// is Z-2 actually free?
const [, price, active, tid] = await pub.readContract({address:COLL,abi,functionName:"seatListing",args:[keccak256(toBytes("Z-2"))]});
console.log(`Z-2: active=${active} tokenId=${tid} price=${formatEther(price)}`);
const wc = createWalletClient({account:buyer,chain,transport:http(RPC)});
try {
  const h = await wc.writeContract({address:COLL,abi,functionName:"buySeat",args:["Z-2"],value:parseEther("0.01"),gas:400000n});
  const r = await pub.waitForTransactionReceipt({hash:h});
  console.log("buy status:", r.status, "gasUsed:", r.gasUsed.toString());
} catch(e){ console.log("buy err:", String(e.message).split("\n")[0]); }
